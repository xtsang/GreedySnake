...
void Food::DrawFood(Snake& csnake_A, Snake& csnake_B, Map& map)//绘制食物
{
    ...
    while (true)
    {
        ...
        {
            if ((point.GetX() == tmp_x && point.GetY() == tmp_y) || (tmp_x == big_x && tmp_y == big_y))  // 普通食物和限时食物位置永远不能相同
            {
                flag = true;
                break;
            }
...
void Food::DrawBigFood(Snake& csnake_A, Snake& csnake_B, Map& map)//绘制限时食物
{
